...
def hotflip_attack(
    ...
    if num_candidates > 1:  # get top k options
        best_k_values, best_k_ids = torch.topk(
            gradient_dot_embedding_matrix, num_candidates, dim=2
        )
        return (
            best_k_values.detach().cpu().numpy()[0],
            best_k_ids.detach().cpu().numpy()[0],
        )
    best_value_at_each_step, best_at_each_step = gradient_dot_embedding_matrix.max(2)
    return (
        best_value_at_each_step[0].detach().cpu().numpy(),
        best_at_each_step[0].detach().cpu().numpy(),
    )
...
def find_hard_salient_phrases(
    ...
    if os.path.exists(output_file):
                    ...
                    for start_idx in range(len(line_split)):
                        for n_idx in range(1, 4):
                            if start_idx + n_idx <= len(line_split):
                                ngram = " ".join(
                                    line_split[start_idx : start_idx + n_idx]
                                )
                                neg_phrases[ngram] += 1
                else:
        ...
        for phrase in neg_phrases:
            neg_over_pos_score = float(neg_phrases[phrase] + smooth_lambda) / (
                pos_phrases[phrase] + smooth_lambda
            )
            pos_over_neg_score = float(pos_phrases[phrase] + smooth_lambda) / (
                neg_phrases[phrase] + smooth_lambda
            )
